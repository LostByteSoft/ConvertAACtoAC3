...
echo -------------------------========================-------------------------
	...
	echo 2024-06-17_Monday_19:24:49
echo -------------------------========================-------------------------
...
echo "The core/code program. Ffmpeg conversion"
	...
	if [ $aac -eq 1 ]; then
	## x264 10b aac
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 21 -preset faster -tune fastdecode -c:a aac "$name".{x264-10b}.{aac}.mkv
	fi
	
	if [ $mp3 -eq 1 ]; then
	## x264 10b mp3
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 21 -preset faster -tune fastdecode -c:a mp3 "$name".{x264-10b}.{mp3}.mkv
	fi
	
	if [ $ac3 -eq 1 ]; then	
	## x264 10b ac3
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 21 -preset faster -tune fastdecode -c:a ac3 "$name".{sdr-x264-10b}.{ac3}.mkv
	fi
	
	if [ $dts -eq 1 ]; then	
	## x264 10b dts
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 21 -preset faster -tune fastdecode -strict experimental -c:a dts "$name".{sdr-x264-10b}.{dts}.mkv
	fi
	
	if [ $copy -eq 1 ]; then
	## x264 10b copy
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 21 -preset faster -tune fastdecode -c:a copy "$name".{sdr-x264-10b}.{copy}.mkv
	fi
	
	if [ $none -eq 1 ]; then
	## x264 10b none
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 21 -preset faster -tune fastdecode -an "$name".{sdr-x264-10b}.{none}.mkv
	fi

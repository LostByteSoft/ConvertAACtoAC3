...
echo "Input name, directory and output name :"
	...
	echo "Output file : "$name".SDR-x264-10b.ac3-48000hz-640k.mkv"
...
ffmpeg -i "$file" -vf zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p10le -c:v libx264 -crf 20 -r:v 30 -preset faster -tune fastdecode -c:a ac3 -ar 48000 -b:a 640k "$NAME".SDR-x264-10b.ac3-48000hz-640k.mkv

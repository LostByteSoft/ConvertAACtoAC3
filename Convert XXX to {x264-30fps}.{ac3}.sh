...
echo "Convert ONE video file to x264 30 fps ac3, all others settings remain unchange"
...
echo "Output file : "$NAME".{x264-30fps}.{ac3}.mkv"
...
ffmpeg -i "$FILE" -c:v libx264 -r:v 30 -c:a ac3 "$NAME".{x264-30fps}.{ac3}.mkv
# ffmpeg -i "$FILE" -c:v libx264 -r:v 30 -c:a copy "$NAME".{x264-30fps}.{copy}.mkv

...
## Software name, what is this, version, informations.
	...
	echo
	echo What it does ?
	echo "Convert ONE video file HDR to SDR Convert HDRtoSDR.{BluRay-2160p}.{SDR-x264-10b}.{dts-5.1}.mkv"
	echo
...
## The code program.
	...
	ffmpeg -i "$file" -vf zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p10le -c:v libx264 -crf 20 -preset faster -tune fastdecode -strict experimental -c:a dts -ar 48000 -b:a 768k "$name".{BluRay-"$res"p}.{SDR-x264-10b}.{dts-5.1}.mkv

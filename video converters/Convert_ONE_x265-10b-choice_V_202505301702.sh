...
echo "-------------------------===== Section $part =====-------------------------"
	...
	if [ $aac -eq 1 ]; then
	## x264 10b aac
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx265 -crf 20 -preset faster -tune fastdecode -c:a aac "$name".{$res.x264-10b}.{aac}.mkv
	fi
	
	if [ $mp3 -eq 1 ]; then
	## x264 10b mp3
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx265 -crf 20 -preset faster -tune fastdecode -c:a mp3 "$name".{$res.x264-10b}.{mp3}.mkv
	fi
	
	if [ $ac3 -eq 1 ]; then	
	## x264 10b ac3
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx265 -crf 20 -preset faster -tune fastdecode -c:a ac3 "$name".{$res.sdr-x264-10b}.{ac3}.mkv
	fi
	
	if [ $dts -eq 1 ]; then	
	## x264 10b dts
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx265 -crf 20 -preset faster -tune fastdecode -strict experimental -c:a dts "$name".{$res.sdr-x264-10b}.{dts}.mkv
	fi
	
	if [ $copy -eq 1 ]; then
	## x264 10b copy
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx265 -crf 20 -preset faster -tune fastdecode -c:a copy "$name".{$res.sdr-x264-10b}.{copy}.mkv
	fi
	
	if [ $none -eq 1 ]; then
	## x264 10b none
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx265 -crf 20 -preset faster -tune fastdecode -an "$name".{$res.sdr-x264-10b}.{none}.mkv
	fi

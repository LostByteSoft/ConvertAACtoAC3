...
echo -------------------------===== Start of bash ====-------------------------
	...
	printf '\033[8;40;120t'		# will resize the window, if needed.
...
## Software name, what is this, version, informations.
	echo "Software name: Convert FOLDER to"
	echo
	echo "What it does ?"
	echo "Convert FOLDER to audio/video"
	echo
	echo "This is a MULTIPLE file conversion"
	echo
...
echo -------------------------========================-------------------------
	...
	echo 2024-02-12_Monday_17:06:04
echo -------------------------========================-------------------------
...
echo -------------------------========================-------------------------
echo "The core/code program. Ffmpeg conversion"
	echo

part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo Finding files...
	#echo Delete /dev/shm/find.txt
	rm "/dev/shm/find.txt" 2> /dev/null
	...
	find "$file" -maxdepth 1 -iname '*.MP4'  >> "/dev/shm/find.txt"
	find "$file" -maxdepth 1 -iname '*.MKV'  >> "/dev/shm/find.txt"
	find "$file" -maxdepth 1 -iname '*.WEBM'  >> "/dev/shm/find.txt"
	find "$file" -maxdepth 1 -iname '*.AVI'  >> "/dev/shm/find.txt"
	echo

part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo show finded files... from /dev/shm/find.txt
	echo	
	cat "/dev/shm/find.txt"
	echo
	
	if [ $logs -eq 1 ]; then
		cat "/dev/shm/find.txt" >> /dev/shm/$random2.txt
		fi

part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"

	lines=$(wc -l < "/dev/shm/find.txt")
...
##parallel -j $entry ffmpeg -i {} -vf scale=1280x720:flags=lanczos,format=yuv420p -c:v libx264 -crf 22 -c:a aac -ac 2 -b:a 224k {.}.{720p-SDR-x264-8b}.{aac-224k}.mkv ::: "$file"/*.*

##parallel -j $entry ffmpeg -i {} -vf format=yuv420p10le -c:v libx264 -crf 22 -preset faster -tune fastdecode -c:a aac -b:a 320k {.}.{7200p-sdr-x264-10b}.{aac-320k}.mkv ::: "$file"/*.*

	debug $?

parallel -a "/dev/shm/find.txt" -j $entry ffmpeg -i {} -vf scale=7680x4320:flags=lanczos,format=yuv420p -r:v 30 -c:v libx264 -crf 22 -preset faster -tune fastdecode -c:a aac -ac 2 -b:a 320k {.}.{4320p-sdr-x264-10b-30f}.{aac-2.0-320k}.mkv

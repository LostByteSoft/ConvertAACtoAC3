...
## Software name, what is this, version, informations.
	echo "Software name: Convert XXX to 720p-x264-8b.aac.sh"
	echo
	echo What it does ?
	echo "Convert ONE video file to 2160p-x264-10b.dts upscale or downscale 4k"
	echo
	echo "Perfect format for 2160p GENERAL application."
	echo
...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "ffmpeg conversion facebook"
	echo

	#ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p -c:v libx264 -crf 25 -c:a aac "$name".{720p-x264-8b}.{aac}.mp4
	#ffmpeg -i "$file" -c:v libx264 -profile:v main -level 3.1 -vf scale=1280:720 -r 30 -b:v 2000k -c:a aac -b:a 128k -ar 48000 -movflags +faststart "$name"_ipad.mp4
	#ffmpeg -i "$file" -c:v libx264 -vf scale=1280:720 -r 30 -b:v 2000k -c:a aac -b:a 160k -ar 48000 -movflags +faststart "$name"_ipad.mp4
	ffmpeg -i "$file" -c:v libx264 -profile:v baseline -level 3.1 -pix_fmt yuv420p -vf scale=1280:720 -r 30 -b:v 2000k -c:a aac -b:a 160k -ar 48000 -movflags +faststart "$name"_ipad.mp4

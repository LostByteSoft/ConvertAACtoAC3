...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "The core/code program. Ffmpeg conversion. Version 1,03"
	## Local var
	aac=0
	ac3=0
	dts=0
	...
	sleep 0.5
	printf '\033[8;26;115t'		## minimum of ? for graphics, will resize the window, if needed.
	sleep 0.5
...
echo "-------------------------===== Section $part =====-------------------------"
	...
	response=$(zenity --height=350 --width=450 --list --radiolist --text "Select sound output option" --column "Select" \
	--column "Sound_Type" FALSE "AAC" FALSE "MP3" FALSE "AC3" FALSE "DTS" TRUE "Copy" FALSE "None")
	...
	if [ $aac -eq 1 ]; then
	## x264 10b aac
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 20 -preset faster -tune fastdecode -c:a aac "$name".{$res.x264-10b}.{aac}.mkv
	fi
	
	if [ $mp3 -eq 1 ]; then
	## x264 10b mp3
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 20 -preset faster -tune fastdecode -c:a mp3 "$name".{$res.x264-10b}.{mp3}.mkv
	fi
	
	if [ $ac3 -eq 1 ]; then	
	## x264 10b ac3
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 20 -preset faster -tune fastdecode -c:a ac3 "$name".{$res.sdr-x264-10b}.{ac3}.mkv
	fi
	
	if [ $dts -eq 1 ]; then	
	## x264 10b dts
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 20 -preset faster -tune fastdecode -strict experimental -c:a dts "$name".{$res.sdr-x264-10b}.{dts}.mkv
	fi
	
	if [ $copy -eq 1 ]; then
	## x264 10b copy
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 20 -preset faster -tune fastdecode -c:a copy "$name".{$res.sdr-x264-10b}.{copy}.mkv
	fi
	
	if [ $none -eq 1 ]; then
	## x264 10b none
	ffmpeg -loglevel error -v error -stats -i "$file" -vf format=yuv420p10le -c:v libx264 -crf 20 -preset faster -tune fastdecode -an "$name".{$res.sdr-x264-10b}.{none}.mkv
	fi

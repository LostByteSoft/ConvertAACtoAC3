...
echo -------------------------========================-------------------------
echo "The core/code program. Video File Checker. Version 1.02"
	echo
	## Informations: https://superuser.com/questions/100288/how-can-i-check-the-integrity-of-a-video-file-avi-mpeg-mp4
	rm "/dev/shm/error.log" 2> /dev/null
...
echo "Get resolution and numbers of audio channel(s) of the multimedia file and lenght."
	...
	echo "${blue}	██████████████████████████████████████████████████████████████████████████████${reset}"
	echo
	echo "	Running job	Resolution	$res	Audio channels	$audio	Lenght time	$lenght"
	echo
	echo "$file"
	echo
	echo "${blue}	██████████████████████████████████████████████████████████████████████████████${reset}"
	echo
	echo "	Analysis started... AND WAIT ! LOG : /dev/shm/error.log"
	echo
	echo ffmpeg -v error -i "$file"

ffmpeg -v error -i "$file" -f null - 2>/dev/shm/error.log

error $?

if [ -s "/dev/shm/error.log" ]; then
	if zenity --question --text="Do you want to view /dev/shm/error.log ?"
		then
			echo ----====----
			echo

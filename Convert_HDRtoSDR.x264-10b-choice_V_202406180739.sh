...
echo Specific softwares variables, you can change theses variables.
	...
	noquit=1
	echo
	echo "Software name: Convert HDRtoSDR-SDR"
	echo
	echo What it does ?
	echo "Convert ONE video file HDR to SDR (With choice option) :"
	echo
	echo "Software name: Convert HDRtoSDR-SDR-x264-10b-AC3"
	echo "Software name: Convert HDRtoSDR-SDR-x264-10b-COPY"
	echo "Software name: Convert HDRtoSDR-SDR-x264-10b-DTS"
	echo "Software name: Convert HDRtoSDR-SDR-x264-10b-NONE"
	echo
...
echo -------------------------========================-------------------------
	...
	echo 2024-06-18_Tuesday_07:39:03
echo -------------------------========================-------------------------
...
echo "The core/code program."
	echo
	echo "ffmpeg conversion ; {sdr-x264-10b}.{ac3.dts.copy.none}"
	echo
...
response=$(zenity --height=250 --width=300 --list --radiolist --text "Select sound output option" --column "Select" --column "Sound_Type" FALSE "AC3" TRUE "DTS" FALSE "Copy" FALSE "None")
	...
	echo Debug data : ac3=$ac3 dts=$dts copy=$copy none=$none
	echo
...
ffmpeg -loglevel error -v error -stats -i "$file" -vf zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p10le \
-c:v libx264 -crf 21 -preset faster -tune fastdecode -c:a ac3 "$name".{sdr-x264-10b}.{ac3}.mkv
	error $?
	fi
...
ffmpeg -loglevel error -v error -stats -i "$file" -vf zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p10le \
-c:v libx264 -crf 21 -preset faster -tune fastdecode -strict experimental -c:a dts -ac 6 "$name".{sdr-x264-10b}.{dts}.mkv
	error $?
	fi
...
ffmpeg -loglevel error -v error -stats -i "$file" -vf zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p10le \
-c:v libx264 -crf 21 -preset faster -tune fastdecode -c:a copy "$name".{sdr-x264-10b}.{copy}.mkv
	error $?
	fi
...
ffmpeg -loglevel error -v error -stats -i "$file" -vf zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p10le \
-c:v libx264 -crf 21 -preset faster -tune fastdecode -an "$name".{sdr-x264-10b}.{none}.mkv
	error $?
	fi

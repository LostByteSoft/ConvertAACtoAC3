...
echo -------------------------===== Start of bash ====-------------------------
	...
	logs=0		# Logging (on desktop) for jobs and info. (0 or 1 do log)
	...
	if [ $logs -eq 1 ]; then
		echo "Log data" >> /dev/shm/$random2.txt
		echo "	" >> /dev/shm/$random2.txt
		echo "Starting time : $now" >> /dev/shm/$random2.txt
		echo "	" >> /dev/shm/$random2.txt
		echo "Debug data start : autoquit=$autoquit debug=$debug error=$error part=$part noquit=$noquit random=$random random2=$random2 logs=$logs" >> /dev/shm/$random2.txt
		echo "	" >> /dev/shm/$random2.txt
	fi
...
## Software name, what is this, version, informations.
	...
	echo
	echo What it does ?
	echo "Convert ALL audio/video file in folder to audio AC3 ac3-48000hz-640k"
	echo
...
echo -------------------------========================-------------------------
	...
	echo 2023-04-28_Friday_06:02:04
echo -------------------------========================-------------------------
...
ext=$(zenity --entry --text="Enter the correct INPUT extension type without the dot (ex: aac , eac3) ?")
...
## Software lead out
	...
	if [ $logs -eq 1 ]; then
		echo "Debug data ending : autoquit=$autoquit debug=$debug error=$error part=$part noquit=$noquit random=$random random2=$random2 logs=$logs" >> /dev/shm/$random2.txt
		echo "	" >> /dev/shm/$random2.txt
		echo "Finish... with numbers of actions : $part" >> /dev/shm/$random2.txt
		echo "This script take $(( SECONDS - start )) seconds to complete." >> /dev/shm/$random2.txt
		date=$(date -d@$(( SECONDS - start )) -u +%H:%M:%S)
		echo "Time needed: $date" >> /dev/shm/$random2.txt
		now=$(date +"%Y-%m-%d_%A_%I:%M:%S")
		echo "Current time : $now" >> /dev/shm/$random2.txt
		echo "	" >> /dev/shm/$random2.txt
		
		cp "/dev/shm/$random2.txt" /$HOME/Desktop/JOB-DONE-$random2.txt
		rm "/dev/shm/$random2.txt" 2> /dev/null
	fi
